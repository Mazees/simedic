    ...
    <main class="mx-auto max-w-3xl p-4 sm:p-8">
        <div class="mb-4">
            <a href="/simedic/histori-transaksi"
                class="inline-flex items-center border-2 border-slate-300 bg-white px-4 py-2 text-sm font-bold text-slate-700 hover:bg-slate-50">
                Kembali
            </a>
        </div>

        <section class="bg-white p-6 sm:p-8 border-t-4 border-cyan-500">
            <div class="border-b-2 border-slate-200 pb-4">
                <p class="text-xs font-bold uppercase tracking-[0.3em] text-slate-500">Struk Pembelian</p>
                <h1 class="mt-2 text-2xl font-bold">SIMEDIC Pharmacy</h1>
            </div>

            <div class="mt-4 grid gap-3 text-sm sm:grid-cols-2">
                <p><span class="text-slate-500 font-semibold">ID Transaksi:</span> #TRX-<?= $idTransaksi > 0 ? $idTransaksi : '-' ?>
                </p>
            </div>
                        ...
                        <?php if (count($detailTransaksi) > 0): ?>
                            <?php foreach ($detailTransaksi as $row): ?>
                                ...
                                <tr class="border-b-2 border-slate-100">
                                    ...
                                    <td class="px-4 py-3">Rp <?= number_format($harga, 0, ',', '.') ?></td>
                                    <td class="px-4 py-3"><?= $qty ?></td>
                                    <td class="px-4 py-3 text-right font-semibold">Rp <?= number_format($subtotal, 0, ',', '.') ?></td>
                                </tr>
                            <?php endforeach; ?>
                        <?php else: ?>
                            <tr class="border-b-2 border-slate-100">
            ...
            <div class="mt-6 border-t-4 border-cyan-500 pt-4 text-sm">
